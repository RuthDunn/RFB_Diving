...
# 
chlorInfo <- info('pmlEsaCCI50OceanColorDaily')
chlorInfo
...
for (i in 1:nrow(files)) {
  ...
  df.dives <- read_csv(paste0("RFB_Diving_Data/BIOT_AxyTrek_Processed/", files[i,], "_dive_stats.csv")) %>%
    mutate(BirdID = i)
  ...
  # Extract the same number of points as there are dives for each trip from:
  # a) The dive track &
  # b) The MCP.
  
  # Run through each trip:
  for (j in 1:length(unique(df.gps$TripID))) {
    
    # j = 2
    
    # Create dataframe of bird id, trip id and date of Trip j, & repeat 3 times:
    tripj.info <- bind_rows(replicate(3, subset(df.dives, TripID == unique(df.dives$TripID)[j])[,c(14, 8, 4)], simplify = F))
    
    # Extract current dive locations:
    tripj.divepoints <- subset(df.dives, TripID == unique(df.dives$TripID[j]))[,c(5, 6)] %>%
      mutate(Value = "Dive.Locs") %>%
      rename(X = 1, Y = 2)
        
    # Subset the gps track of Trip j:
    tripj <- subset(df.gps, Trip == unique(df.gps$Trip)[j])
    ...
    tripj.pspointsa <- st_line_sample(x = sf_linestring(obj = df.gps, x = "Lon", y = "Lat"),
                                      n = nrow(tripj.info)/3, type = "random")
    ...
    tripj.enviro <- cbind(tripj.info, rbind(tripj.divepoints, tripj.pspointsa, tripj.pspointsb)) %>%
      ...
      mutate(Depth = rxtracto(bathyInfo, parameter = 'elevation',
                            xcoord = X, ycoord = Y,
                            xlen = .2, ylen = .2, progress_bar = TRUE)[[1]]) %>%
      mutate(Chlor = rxtracto(chlorInfo, parameter = 'chlor_a',
                              xcoord = X, ycoord = Y, tcoord = as.Date(DateTime),
                              xlen = .2, ylen = .2, progress_bar = TRUE)[[1]])
  ...
  rm(tripj.divepoints, tripj.pspointsa, tripj.pspointsb, tripj, tripj.info, bathyInfo, chlorInfo, sstInfo)
